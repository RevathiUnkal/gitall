...
class StudentCourseInformation {
    // Map semester number -> list of courses and marks
    private Map<Integer, List<CourseMark>> semesterCourses;

    public StudentCourseInformation() {
        semesterCourses = new HashMap<>();
    }

    public void addCourse(int semesterNumber, String courseName, int marks) {
        semesterCourses.computeIfAbsent(semesterNumber, k -> new ArrayList<>())
            .add(new CourseMark(courseName, marks));
    }

    public void displayCourseInformation() {
        if (semesterCourses.isEmpty()) {
            System.out.println("No course information available.");
            return;
        }

        for (Map.Entry<Integer, List<CourseMark>> entry : semesterCourses.entrySet()) {
    ...
    private static class CourseMark {
        String courseName;
        int marks;
...
public class StudentInfo {
    public static void main(String[] args) {
        try {
            Student student = new Student("Alice Johnson", "15-05-2000");
            student.displayStudentInfo();
            System.out.println();

            StudentCourseInformation courses = new StudentCourseInformation();
            courses.addCourse(1, "Mathematics", 87);
            courses.addCourse(1, "Physics", 92);
            courses.addCourse(1, "Chemistry", 85);
            courses.addCourse(2, "Data Structures", 90);
            courses.addCourse(2, "Algorithms", 88);
            courses.addCourse(2, "Operating Systems", 93);
